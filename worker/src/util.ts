...
function formatStatusChangeNotification(
  ...
  if (isUp) {
    return {
      ...
      body: `服务已恢复正常，本次故障持续 ${downtimeDuration} 分钟。`,
    }
  } else if (timeNow == timeIncidentStart) {
    return {
      title: `🔴 ${monitor.name} 当前不可用`,
      body: `服务在 ${timeNowFormatted} 检测到不可用。原因：${reason || '未说明'}`,
    }
  } else {
    return {
      title: `🔴 ${monitor.name} 仍然不可用`,
      body: `服务自 ${timeIncidentStartFormatted} 起不可用，已持续 ${downtimeDuration} 分钟。原因：${reason || '未说明'}`,
    }
  }
}
